...
from GOKU_USER.dB.filter_db import add_filter, get_filter, list_filter, rem_filter
...
@GOKU_USERBOT_cmd(pattern="addfilter( (.*)|$)")
async def af(e):
    ...
    GOKU_USERBOT_bot.add_handler(filter_func, events.NewMessage())


@GOKU_USERBOT_cmd(pattern="remfilter( (.*)|$)")
async def rf(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
...
@GOKU_USERBOT_cmd(pattern="listfilter$")
async def lsnote(e):
