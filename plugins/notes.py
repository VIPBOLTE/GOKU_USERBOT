...
from GOKU_USER.dB.notes_db import add_note, get_notes, list_note, rem_note
...
@GOKU_USERBOT_cmd(pattern="addnote( (.*)|$)", admins_only=True)
async def an(e):
    ...
    GOKU_USERBOT_bot.add_handler(notes, events.NewMessage())


@GOKU_USERBOT_cmd(pattern="remnote( (.*)|$)", admins_only=True)
async def rn(e):
    wrd = (e.pattern_match.group(1).strip()).lower()
    chat = e.chat_id
    if not wrd:
...
@GOKU_USERBOT_cmd(pattern="listnote$", admins_only=True)
async def lsnote(e):
